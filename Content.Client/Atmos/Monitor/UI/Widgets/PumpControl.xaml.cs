...
public sealed partial class PumpControl : BoxContainer
{
    ...
    public PumpControl(GasVentPumpData data, string address)
    {
        ...
        // Begin L5 - flowmos
        foreach (var value in Enum.GetValues<GasVentPumpData.VentPumpFlowmos>())
        {
            _flowmosMode.AddItem(value.ToString());
        }

        _flowmosMode.SelectId((int) _data.VentFlowmosMode);
        _flowmosMode.OnItemSelected += args =>
        {
            _flowmosMode.SelectId(args.Id);
            _data.VentFlowmosMode = (GasVentPumpData.VentPumpFlowmos) args.Id;

            // Update thresholds to be reasonable defaults for the flowmos mode. This ignores the current air alarm
            // mode, but this matches the behavior of e.g., adding a scrubber to an alarm that's in wide filtering and
            // having the added scrubber not be in widenet.
            switch (_data.VentFlowmosMode)
            {
                case GasVentPumpData.VentPumpFlowmos.Inlet:
                    _data.ExternalPressureBound = Atmospherics.OneAtmosphere + Atmospherics.AirVentPressureDelta;
                    _data.PumpDirection = VentPumpDirection.Releasing;
                    break;
                case GasVentPumpData.VentPumpFlowmos.Outlet:
                    _data.ExternalPressureBound = Atmospherics.OneAtmosphere - Atmospherics.AirVentPressureDelta;
                    _data.PumpDirection = VentPumpDirection.Siphoning;
                    break;
                case GasVentPumpData.VentPumpFlowmos.Default:
                    _data.ExternalPressureBound = Atmospherics.OneAtmosphere;
                    _data.PumpDirection = VentPumpDirection.Releasing;
                    break;
            }
    ...
    public void ChangeData(GasVentPumpData data)
    {
        ...
        // L5 - flowmos
        _data.VentFlowmosMode = data.VentFlowmosMode;
        _flowmosMode.SelectId((int) _data.VentFlowmosMode);
    }
}
